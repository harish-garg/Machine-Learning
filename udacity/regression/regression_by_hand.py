...
def compute_regression(sleep,scores):
    ...
    slope = sum([x * y for (x, y) in zip(normalized_scores, normalized_sleep)]) /  sum([x * x for x in normalized_sleep])
    ...
    # y = slope*x - slope*avg_sleep + avg_scores
    b = sum([a / (slope * b) for a,b in zip(normalized_scores, normalized_sleep)])
